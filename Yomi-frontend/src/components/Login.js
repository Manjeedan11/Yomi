    ...
    if (validate()) {
      ...
      try {
        const response = await fetch(
          "https://yomi-backend.vercel.app/api/auth",
          {
            method: "POST",
            headers: {
  ...
  return (
    <div className="flex w-full h-full">
      ...
      <div className="absolute inset-y-0 right-0 w-1/3 bg-background bg-opacity-100 flex flex-col items-center justify-center  rounded-l-lg mt-4 mb-4 mr-8">
        ...
        <a href="/register" className="text-highlight text-s underline pt-2">
          I dont have an account
        </a>
        <a href="/" className="text-highlight text-xs underline pt-2">
          return home
        </a>
      </div>
      <div className="absolute bottom-0 left-0 p-4 text-gray-100 text-sm">
        © Artwork by{" "}
        <a href="https://www.pixiv.net/en/artworks/95148272">粉帮老大</a>
      </div>
    </div>
  );
}
